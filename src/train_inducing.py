...
def var_loglik_fun(q, batch, apply_fn, unravel_fn, rng, num_mc_samples):
    ...
    for i in range(num_mc_samples):
        ...
        log_p_data = nl_likelihood_fun(apply_fn, theta_sample, batch) # todo from train_map import nl_likelihood_fun
        log_sum += log_p_data
...
def naive_objective(xind, dataset, state, alpha, rng, num_mc_samples, full_set_size=None, reg_coeff=0): # todo reg_coeff currently defaults to 0
    q,unravel_fn = posterior_lla(state, prior_std=alpha, 
                                 x=xind, y=None, # ? explicitly pass no labels
                                 full_set_size=full_set_size, return_unravel_fn=True)
...
def train_inducing_points(map_model_state, xinit, 
    ...
    def get_next_sample(num_batches=1):
        ...
        return sample
    dataset_sample = get_next_sample(num_batches=20) # ! estimate constraints from a 20 batch sample (i.e. data support)
