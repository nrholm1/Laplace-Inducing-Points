...
def test_posterior_lla(small_model_state, regression_1d_data):
    ...
    post_dist = posterior_lla(small_model_state, prior_std, X, w, y)
    
    _, flat_params_map, _ = compute_curvature_approx(
        small_model_state, regression_1d_data, prior_std, w, return_Hinv=False
    )
...
def test_predict_lla(small_model_state, regression_1d_data):
    ...
    pred_dist = predict_lla(small_model_state, xnew, X, w, y, prior_std=prior_std)
