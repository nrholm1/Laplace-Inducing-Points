...
if __name__ == '__main__':
    ...
    from src.ggn2 import compute_W_vps, compute_ggn_vp, compute_ggn_dense
    ...
    GGN_data_dense =  compute_ggn_dense(REAL_DATA, flat_params, model.apply, unravel_fn)
    GGN_ip_dense   =  compute_ggn_dense(IP_DATA, flat_params, model.apply, unravel_fn)

    GGN_data =  compute_ggn_vp(REAL_DATA, flat_params, model.apply, unravel_fn)
    GGN_ip   =  compute_ggn_vp(IP_DATA, flat_params, model.apply, unravel_fn)
    W, WT    =  compute_W_vps( REAL_DATA, flat_params, model.apply, unravel_fn)
    Wip, WTip=  compute_W_vps( IP_DATA, flat_params, model.apply, unravel_fn)
    ...
    est_trace = estimate(lambda v: alpha*v + beta*GGN_data(v), _key(+45_88_88_88_88))
